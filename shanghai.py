...
scene.set_directional_light((1, 1, 0.75), 0.5, (1.0, 0.9, 0.8))
...
def initialize():
    for x, z in ti.ndrange((-64, 64), (-64, 64)):
        ...
        if h >= 0: fill(ivec3(x, -64, z), ivec3(1, 4 + h, 1), 1, vec3(0.25, 0.5, 0.25), 0.1)
    for x, z in ti.ndrange((-64, 64), (-64, 64)):
        if scene.get_voxel(ivec3(x, -64, z))[0] == 0:
            ...
            scene.get_voxel(ivec3(x1, -64, z1))[0] > 0): set(ivec3(x, -63, z), 1, vec3(0.05, 0.1, 0.3), vec3(0, 0, 0.1))
    for x, z in ti.ndrange((-64, 64), (-64, 64)):
        if scene.get_voxel(ivec3(x, -64, z))[0] == 0 and scene.get_voxel(ivec3(x, -63, z))[0] > 0:
            set(ivec3(x, -64, z), 1, vec3(0.05, 0.1, 0.3), vec3(0, 0, 0.1))
        else:
            fill(ivec3(x, -64, z), ivec3(1, 3, 1), 1, vec3(0.7, 0.5, 0.3), 0.1)
            set(ivec3(x, -61, z), 1, vec3(0.25, 0.5, 0.25), 0.1)
    for x, z in ti.ndrange((-64, 64), (-64, 64)):
    ...
    for x, z in ti.ndrange((-64, 64), (-64, 64)):
        ...
        if y <= -64 + 2 and ti.random() < 0.02: set(ivec3(x, y - 1, z), 2, vec3(0.3), 0.7)
        elif y >= -64 + 4 and ti.random() < 0.008: set(ivec3(x, y, z), 1, vec3(0.8, 0.8 * ti.random() ** 4, 0), 0.2)
